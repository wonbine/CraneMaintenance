...
export default function Dashboard() {
  ...
  const DonutChart = ({ data, title, total }: { data: any[], title: string, total: number }) => (
    <div className="relative w-32 h-32 mx-auto">
      <svg className="w-32 h-32 transform -rotate-90">
        <circle cx="64" cy="64" r="52" fill="transparent" stroke="#e5e7eb" strokeWidth="8"/>
        {data.map((item, index) => {
          const percentage = (item.count / total) * 100;
          const strokeDasharray = `${(percentage / 100) * 326.7} 326.7`;
          const strokeDashoffset = -326.7 * data.slice(0, index).reduce((acc, curr) => acc + (curr.count / total), 0);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 font-['IBM_Plex_Sans']">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">크레인 정비 대시보드</h1>
        <p className="text-gray-600">실시간 크레인 상태 모니터링 및 정비 관리</p>
      </div>
            ...
            <CardContent>
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="space-y-2">
                  <div className="text-center">
                    <p className="text-2xl font-bold text-blue-600">{craneData.dailyRepair.totalHours}</p>
          ...
          <Card className="shadow-lg border-0 rounded-xl">
            ...
            <CardContent>
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="space-y-2">
                  <div className="text-center">
                    <p className="text-2xl font-bold text-red-600">{craneData.emergencyRepair.totalHours}</p>
                    <p className="text-sm text-gray-500">총 시간</p>
                  </div>
                  <div className="space-y-1">
          ...
          <Card className="shadow-lg border-0 rounded-xl">
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center">
                  <p className="text-xl font-bold text-blue-600">{craneData.metrics.avgRepairTime}</p>
                  <p className="text-xs text-gray-500">평균 수리시간</p>
                </div>
                <div className="text-center">
                  <p className="text-xl font-bold text-green-600">{craneData.metrics.operationRate}</p>
                  <p className="text-xs text-gray-500">가동률</p>
                </div>
                <div className="text-center">
                  <p className="text-xl font-bold text-purple-600">{craneData.metrics.mtbf}</p>
                  <p className="text-xs text-gray-500">MTBF</p>
                </div>
                <div className="text-center">
                  <p className="text-xl font-bold text-orange-600">{craneData.metrics.reliability}</p>
